...
begin
  ...
  if not found then
    insert into public.records (
      owner_id,id,kind,title,description,status,date,end_date,subject,predicate,value,tags,related_ids,sync_meta
    ) values (
      ...
      array(select jsonb_array_elements_text(coalesce(p_record->'related_ids','[]'::jsonb))), p_versions
