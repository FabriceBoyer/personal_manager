...
import {createRecord, importRecords, listRecords, updateRecord} from './records';
import './styles.css';
import './theme.css';
import './auth.css';
...
function App(){
  ...
  const load=async()=>{setLoading(true);try{setRecords(await listRecords());setError('')}catch(e){setError(e.message)}finally{setLoading(false)}};
  useEffect(()=>{
...
function HelpView({locale,setView,reload}){
 const fr=locale==='fr';
