...
import {Archive, ArchiveRestore, BookOpen, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Circle, Clock3, Command, Database, Download, Globe2, HelpCircle, LayoutDashboard, Link2, ListTodo, MapPin, MoonStar, Palette, Pencil, Plus, Search, ShieldCheck, Sparkles, Sun, Trash2, WandSparkles, X, Zap} from 'lucide-react';
import {supabase} from './supabase';
import {archiveRecord, createRecord, deleteRecord, importRecords, listRecords, resetRecords, subscribeToRecordChanges, syncPendingRecords, updateRecord} from './records';
...
const nav = [['overview','today',LayoutDashboard],['actions','actions',ListTodo],['calendar','calendar',CalendarDays],['journal','journal',CheckCircle2],['knowledge','knowledge',BookOpen],['archives','archives',Archive],['help','help',HelpCircle]];
const messages={
 fr:{today:"Aujourd’hui",actions:'Actions',calendar:'Calendrier',journal:'Journal',knowledge:'Connaissances',archives:'Archives',help:'Aide',search:'Rechercher partout…',add:'Ajouter',workspace:'ESPACE PERSONNEL',sync:'Synchronisé',now:"À l’instant",personal:'Espace personnel',
 ...
 en:{today:'Today',actions:'Actions',calendar:'Calendar',journal:'Journal',knowledge:'Knowledge',archives:'Archives',help:'Help',search:'Search everything…',add:'Add',workspace:'PERSONAL SPACE',sync:'Synced',now:'Just now',personal:'Personal space',
...
function App(){
  ...
  const [accent,setAccent]=useState(()=>localStorage.getItem('memoire-accent')||'#e86d4d');
  const [installPrompt,setInstallPrompt]=useState(null);
  const [syncState,setSyncState]=useState({status:navigator.onLine?'syncing':'offline',pending:0});
  useEffect(()=>{document.documentElement.dataset.theme=theme;localStorage.setItem('memoire-theme',theme)},[theme]);
  useEffect(()=>{document.documentElement.style.setProperty('--accent',accent);document.querySelector('meta[name="theme-color"]')?.setAttribute('content',accent);localStorage.setItem('memoire-accent',accent)},[accent]);
  useEffect(()=>{const capture=e=>{e.preventDefault();setInstallPrompt(e)};const installed=()=>setInstallPrompt(null);window.addEventListener('beforeinstallprompt',capture);window.addEventListener('appinstalled',installed);return()=>{window.removeEventListener('beforeinstallprompt',capture);window.removeEventListener('appinstalled',installed)}},[]);
  ...
  useEffect(()=>{const listener=e=>setSyncState(e.detail);window.addEventListener('memoire-sync',listener);return()=>window.removeEventListener('memoire-sync',listener)},[]);
  useEffect(()=>{
    if(!session?.user.id)return;
    const refresh=()=>load({silent:true});
    const reconnect=()=>syncPendingRecords().then(refresh).catch(refresh);
    const unsubscribe=subscribeToRecordChanges(session.user.id,refresh,status=>status==='SUBSCRIBED'&&setSyncState(current=>current.pending?current:{...current,status:navigator.onLine?'synced':'offline'}));
    const visibility=()=>document.visibilityState==='visible'&&navigator.onLine&&refresh();
    const disconnect=()=>setSyncState(current=>({...current,status:'offline'}));
    window.addEventListener('online',reconnect);window.addEventListener('offline',disconnect);window.addEventListener('focus',visibility);document.addEventListener('visibilitychange',visibility);
    const interval=setInterval(()=>navigator.onLine&&refresh(),30000);
    return()=>{unsubscribe();clearInterval(interval);window.removeEventListener('online',reconnect);window.removeEventListener('offline',disconnect);window.removeEventListener('focus',visibility);document.removeEventListener('visibilitychange',visibility)};
  },[session?.user.id]);
  const filtered=useMemo(()=>records.filter(r=>(r.title+' '+r.description+' '+r.tags?.join(' ')+' '+r.id).toLowerCase().includes(query.toLowerCase())&&(view==='archives'?Boolean(r.archivedAt):!r.archivedAt)),[records,query,view]);
  const title={overview:tr(locale,'today'),actions:locale==='fr'?'Toutes les actions':'All actions',calendar:tr(locale,'calendar'),journal:locale==='fr'?'Journal des actions':'Action journal',knowledge:locale==='fr'?'Base de connaissances':'Knowledge base',archives:tr(locale,'archives'),help:tr(locale,'help')}[view];
  const subtitle={overview:tr(locale,'overviewSub'),actions:tr(locale,'actionsSub'),calendar:tr(locale,'calendarSub'),journal:tr(locale,'journalSub'),knowledge:tr(locale,'knowledgeSub'),archives:locale==='fr'?'Retrouvez, restaurez ou supprimez vos éléments archivés.':'Find, restore, or delete your archived items.',help:tr(locale,'helpSub')}[view];
  ...
  return <div className="app">
    <aside>
      ...
      <nav>{nav.map(([id,label,Icon])=><button key={id} aria-label={tr(locale,label)} className={view===id?'active':''} onClick={()=>setView(id)}><Icon size={19}/><span>{tr(locale,label)}</span>{id==='actions'&&<em>{records.filter(r=>r.kind==='action'&&r.status!=='done'&&!r.archivedAt).length}</em>}</button>)}</nav>
      <div className="sideFoot"><div className={`sync ${syncState.status}`}><i></i><span><b>{{offline:locale==='fr'?'Hors connexion':'Offline',syncing:locale==='fr'?'Synchronisation…':'Syncing…',error:locale==='fr'?'Synchronisation différée':'Sync delayed',synced:tr(locale,'sync')}[syncState.status]||tr(locale,'sync')}</b><small>{syncState.pending?`${syncState.pending} ${locale==='fr'?'modification(s) en attente':'pending change(s)'}`:syncState.status==='offline'?(locale==='fr'?'Mode local actif':'Local mode active'):tr(locale,'now')}</small></span></div><button className="profile" onClick={()=>supabase.auth.signOut()} title={locale==='fr'?'Se déconnecter':'Sign out'}><span>{session.user.email?.slice(0,2).toUpperCase()}</span><div><b>{session.user.email}</b><small>{locale==='fr'?'Se déconnecter':'Sign out'}</small></div></button></div>
    </aside>
    <main>
      <header><div className="search"><Search size={18}/><input aria-label={tr(locale,'search')} placeholder={tr(locale,'search')} value={query} onChange={e=>setQuery(e.target.value)}/><kbd>⌘ K</kbd></div><div className="headerActions"><label className="accentPicker" title={locale==='fr'?"Couleur d’accent":'Accent color'}><Palette/><input type="color" aria-label={locale==='fr'?"Couleur d’accent":'Accent color'} value={accent} onChange={e=>setAccent(e.target.value)}/></label><button className="roundAction" aria-label={tr(locale,'autoTheme')} title={tr(locale,'autoTheme')} onClick={()=>setTheme(theme==='auto'?'light':theme==='light'?'dark':'auto')}>{theme==='light'?<Sun/>:theme==='dark'?<MoonStar/>:<WandSparkles/>}</button><button className="roundAction language" aria-label={locale==='fr'?'Switch to English':'Passer en français'} onClick={()=>setLocale(locale==='fr'?'en':'fr')}><Globe2/><span>{locale.toUpperCase()}</span></button><button className="add" onClick={()=>setModal(true)}><Plus size={18}/>{tr(locale,'add')}</button></div></header>
...
function RecordList({view,records,markDone,locale,onOpen}){
 const kinds={actions:['action','event'],journal:['journal'],knowledge:['fact'],archives:['action','event','journal','fact']}[view]; const list=records.filter(r=>kinds.includes(r.kind));
...
function HelpView({locale,setView,reload,onReset,installPrompt,install}){
 const fr=locale==='fr';
 ...
 const handleExport=async()=>{
  ...
  try{
   ...
   link.href=url;link.download=`memoire-${localISO(new Date())}.json`;
   document.body.appendChild(link);link.click();link.remove();
   setTimeout(()=>URL.revokeObjectURL(url),60_000);
   setExportMessage(fr?`${items.length} objet(s) exporté(s).`:`${items.length} item(s) exported.`);
  }catch(error){setExportMessage(error.message)}finally{setBusy(false)}
 };
 ...
 <section className="helpSection dataSection"><h2>{fr?'Installer Mémoire':'Install Mémoire'}</h2><p>{fr?"Installez le site comme une application. Sur iPhone ou iPad, utilisez Partager puis Sur l’écran d’accueil.":'Install the site as an app. On iPhone or iPad, use Share then Add to Home Screen.'}</p>{installPrompt?<button className="dataButton" onClick={install}><Download/>{fr?"Installer l’application":'Install the app'}</button>:<p className="installHint">{fr?'Le navigateur proposera l’installation si elle est disponible.':'Your browser will offer installation when available.'}</p>}</section>
 ...
 <section className="helpSection dataSection"><h2>{fr?'Importer ou exporter mes données':'Import or export my data'}</h2><p>{fr?'Téléchargez tous les objets de votre compte au format JSON, ou sélectionnez un fichier JSON pour les importer. Les identifiants déjà présents sont ignorés à l’import.':'Download all items in your account as JSON, or select a JSON file to import. Existing IDs are skipped during import.'}</p><div className="dataActions"><label className="importButton">{fr?'Importer un fichier JSON':'Import a JSON file'}<input type="file" accept="application/json,.json" onChange={handleImport}/></label><button className="dataButton" disabled={busy} onClick={handleExport}>{fr?'Exporter en JSON':'Export as JSON'}</button></div>{importMessage&&<p role="status">{importMessage}</p>}{exportMessage&&<p role="status">{exportMessage}</p>}</section>
...
function EntityForm({records,close,saved,locale,record=null}){
 const fr=locale==='fr';
 const editing=Boolean(record);
 const [kind,setKind]=useState(record?.kind||'action'),[form,setForm]=useState(record?{title:record.title||'',description:record.description||'',date:record.date||'',endDate:record.endDate||'',subject:record.subject||'',predicate:record.predicate||'',value:record.value||'',status:record.status||'active',tags:(record.tags||[]).join(', '),relatedIds:record.relatedIds||[]}:{title:'',description:'',date:'',endDate:'',subject:'',predicate:'',value:'',tags:'',relatedIds:[]}),[error,setError]=useState(''),[saving,setSaving]=useState(false);
 const change=e=>setForm({...form,[e.target.name]:e.target.value});
 const submit=async e=>{e.preventDefault();setSaving(true);setError('');const payload={...form,kind,status:form.status||(kind==='journal'?'done':kind==='event'?'scheduled':kind==='fact'?'known':'active'),tags:form.tags.split(',').map(x=>x.trim()).filter(Boolean),relatedIds:form.relatedIds||[]};const changes=editing?Object.fromEntries(Object.entries(payload).filter(([key,value])=>JSON.stringify(value)!==JSON.stringify(record[key]??(['tags','relatedIds'].includes(key)?[]:'')))):payload;try{editing?await updateRecord(record.id,changes):await createRecord(payload);saved()}catch(error){setError(error.message)}finally{setSaving(false)}};
 const choices=records.filter(r=>r.id!==record?.id);
 return <div className="overlay" onMouseDown={e=>e.target===e.currentTarget&&close()}><form className="modal" onSubmit={submit}><div className="modalHead"><div><span className="eyebrow">{editing?(fr?'MODIFIER':'EDIT'):(fr?'NOUVEL OBJET':'NEW OBJECT')}</span><h2>{editing?(fr?'Modifier cet objet':'Edit this item'):(fr?'Ajouter à votre mémoire':'Add to your memory')}</h2></div><button aria-label={fr?'Fermer':'Close'} type="button" onClick={close}><X/></button></div><div className="kindPicker">{Object.entries(kindMeta).map(([id,m])=>{const Icon=m.icon;const name=fr?m.label:{action:'Action',event:'Appointment',journal:'Journal',fact:'Knowledge'}[id];return <button type="button" className={kind===id?'selected '+m.color:''} key={id} onClick={()=>setKind(id)}><Icon size={18}/>{name}</button>})}</div><div className="autoId"><ShieldCheck/><span><b>{editing?record.id:(fr?'Identifiant géré automatiquement':'Identifier managed automatically')}</b><small>{editing?(fr?"L’identifiant reste inchangé.":'The identifier remains unchanged.'):(fr?'Un identifiant unique sera attribué à la création.':'A unique identifier will be assigned when created.')}</small></span></div><label>{fr?'Titre':'Title'}<input name="title" value={form.title} onChange={change} required/></label>{kind==='fact'?<div className="factFields"><label>{fr?'Sujet':'Subject'}<input name="subject" value={form.subject} onChange={change}/></label><label>{fr?'Relation':'Relationship'}<input name="predicate" value={form.predicate} onChange={change}/></label><label>{fr?'Valeur':'Value'}<input name="value" value={form.value} onChange={change}/></label></div>:<><label>Notes<textarea name="description" value={form.description} onChange={change}/></label><div className="factFields"><label>{fr?'Date et heure':'Date and time'}<input type="datetime-local" name="date" value={form.date} onChange={change}/></label>{kind==='event'&&<label>{fr?'Fin':'End'}<input type="datetime-local" name="endDate" value={form.endDate} onChange={change}/></label>}<label>{fr?'Statut':'Status'}<input name="status" value={form.status||''} onChange={change}/></label></div></>}<label>Tags <small>{fr?'séparés par des virgules':'comma separated'}</small><input name="tags" value={form.tags} onChange={change}/></label><label>{fr?'Relier à un objet':'Link to an object'}<select multiple value={form.relatedIds} onChange={e=>setForm({...form,relatedIds:[...e.target.selectedOptions].map(o=>o.value)})}>{choices.map(r=><option value={r.id} key={r.id}>{r.id} — {r.title}</option>)}</select></label>{error&&<div className="error">{error}</div>}<div className="modalActions"><button type="button" onClick={close}>{fr?'Annuler':'Cancel'}</button><button className="primary" disabled={saving}>{saving?(fr?'Enregistrement…':'Saving…'):(editing?(fr?'Enregistrer':'Save'):(fr?'Créer l’objet':'Create item'))}</button></div></form></div>
}
function CreateModal(props){return <EntityForm {...props} saved={props.created}/>}
function DetailModal({record,records,close,locale,saved}){const [editing,setEditing]=useState(false),[confirmDelete,setConfirmDelete]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState('');const fr=locale==='fr';if(editing)return <EntityForm record={record} records={records} locale={locale} close={()=>setEditing(false)} saved={saved}/>;const meta=kindMeta[record.kind];const Icon=meta.icon;const related=(record.relatedIds||[]).map(id=>records.find(r=>r.id===id)).filter(Boolean);const archive=async()=>{setBusy(true);setError('');try{await archiveRecord(record.id,!record.archivedAt);await saved()}catch(e){setError(e.message);setBusy(false)}};const remove=async()=>{setBusy(true);setError('');try{await deleteRecord(record.id);await saved()}catch(e){setError(e.message);setBusy(false)}};return <div className="overlay detailOverlay" onMouseDown={e=>e.target===e.currentTarget&&close()}><article className="detailModal"><div className="detailHead"><span className={'detailIcon '+meta.color}><Icon/></span><div><span className="eyebrow">{fr?meta.label:{action:'ACTION',event:'APPOINTMENT',journal:'JOURNAL',fact:'KNOWLEDGE'}[record.kind]}</span><h2>{record.title}</h2><code>{record.id}</code></div><div className="detailButtons"><button aria-label={fr?'Modifier':'Edit'} onClick={()=>setEditing(true)}><Pencil/></button><button aria-label={fr?'Fermer':'Close'} onClick={close}><X/></button></div></div><div className="detailBody">{record.description&&<section><small>NOTES</small><p>{record.description}</p></section>}{record.kind==='fact'&&<section className="factStatement"><span>{record.subject}</span><b>{record.predicate}</b><span>{record.value}</span></section>}<div className="detailGrid"><section><small>{fr?'STATUT':'STATUS'}</small><strong>{record.status}</strong></section>{record.date&&<section><small>DATE</small><strong>{dateLabel(record.date,locale)} · {timeLabel(record.date)}</strong></section>}<section><small>{fr?'CRÉÉ LE':'CREATED'}</small><strong>{dateLabel(record.createdAt,locale)}</strong></section></div>{record.tags?.length>0&&<section><small>TAGS</small><div className="detailTags">{record.tags.map(tag=><span key={tag}>#{tag}</span>)}</div></section>}{related.length>0&&<section><small>{fr?'OBJETS LIÉS':'RELATED OBJECTS'}</small><div className="relatedList">{related.map(item=><div key={item.id}><b>{item.title}</b><code>{item.id}</code></div>)}</div></section>}{error&&<div className="error" role="alert">{error}</div>}<div className="detailActions"><button disabled={busy} onClick={archive}>{record.archivedAt?<ArchiveRestore/>:<Archive/>}{record.archivedAt?(fr?'Restaurer':'Restore'):(fr?'Archiver':'Archive')}</button><button className="deleteButton" disabled={busy} onClick={()=>setConfirmDelete(true)}><Trash2/>{fr?'Supprimer':'Delete'}</button></div></div>{confirmDelete&&<div className="confirmLayer"><div className="confirmCard" role="alertdialog" aria-modal="true"><h2>{fr?'Supprimer cet élément ?':'Delete this item?'}</h2><p>{fr?'Cette action est définitive. Les liens vers cet élément seront également retirés.':'This cannot be undone. Links to this item will also be removed.'}</p><div className="modalActions"><button disabled={busy} onClick={()=>setConfirmDelete(false)}>{fr?'Annuler':'Cancel'}</button><button className="dangerButton" disabled={busy} onClick={remove}>{busy?'…':(fr?'Supprimer définitivement':'Delete permanently')}</button></div></div></div>}</article></div>}
