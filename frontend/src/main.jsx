...
function EntityForm({records,close,saved,locale,record=null}){
 ...
 const submit=async e=>{e.preventDefault();setSaving(true);setError('');const payload={...form,kind,status:form.status||(kind==='journal'?'done':kind==='event'?'scheduled':kind==='fact'?'known':'active'),tags:form.tags.split(',').map(x=>x.trim()).filter(Boolean),relatedIds:form.relatedIds||[]};try{editing?await updateRecord(record.id,payload):await createRecord(payload);saved()}catch(error){setError(error.message)}finally{setSaving(false)}};
